...
class GPT(nn.Module):
    ...
    def generate(
        ...
        **hf_generate_kwargs,
        ...
        # Compute embeddings for conditioning and text, and prepare the input sequence
        # This concatenates style embeddings and text embeddings with proper positional encoding
        gpt_inputs = self.compute_embeddings(cond_latents, text_inputs)

        # Generate audio tokens autoregressively using HuggingFace's generate method
        gen = self.gpt_inference.generate(
            gpt_inputs,    # Combined condition + text embeddings as context
            bos_token_id=self.start_audio_token,    # Token to start audio generation
            pad_token_id=self.stop_audio_token,     # Token used for padding
            eos_token_id=self.stop_audio_token,     # Token that signals end of generation
            # Maximum length = maximum allowed mel tokens + length of input context
            max_length=self.max_gen_mel_tokens + gpt_inputs.shape[-1],
            **hf_generate_kwargs,  # Pass through generation parameters
        )

        # If detailed generation info is requested (for debugging/analysis)
        if "return_dict_in_generate" in hf_generate_kwargs:
            # Return both the generated sequence (without input context) and full generation info
            return gen.sequences[:, gpt_inputs.shape[1]:], gen
        
        # For normal inference, return only the generated audio tokens
        # Slice off the input context (gpt_inputs) to get only the generated part
