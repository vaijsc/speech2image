...
class GPTTrainer(BaseTTS):
    ...
    def format_batch_on_device(self, batch):
        ...
        if self.config.audio.sample_rate != self.config.audio.dvae_sample_rate:
            dvae_wav = torchaudio.functional.resample( # wav input for dvae: 22k sample rate
                batch["wav"],
            ...
            dvae_wav = batch["wav"]
        dvae_mel_spec = self.torch_mel_spectrogram_dvae(dvae_wav) # convert wav to melspec
        codes = self.dvae.get_codebook_indices(dvae_mel_spec) # get discrete codes using dvae
