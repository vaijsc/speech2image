...
class Xtts(BaseTTS):
    ...
    def get_conditioning_latents(
        ...
        for file_path in audio_paths:
            ...
            speaker_embedding = self.get_speaker_embedding(audio, load_sr) # NOTE
        ...
        gpt_cond_latents = self.get_gpt_cond_latents( # NOTE
    ...
    def inference(
        ...
        **hf_generate_kwargs,
        ...
        for sent in text:
            ...
            ), f" ❗ XTTS can only generate text with a maximum of 400 tokens, but receiving {text_tokens} correponds to text {sent} \n full text {text}"

            with torch.no_grad():
                gpt_codes = self.gpt.generate( # (1, 127) - 127 num of codes, input dependent
                    ...
                    num_return_sequences=self.gpt_batch_size, # 1
    ...
    def batch_inference(
        ...
        **hf_generate_kwargs,
        ...
        gpt_cond_latents = gpt_cond_latents.to(self.device)
        ...
        text_tokens = [
            torch.IntTensor(self.tokenizer.encode(text, lang=language))
            for text in texts
        ] 
        text_lengths = torch.tensor([len(t) for t in text_tokens], device=self.device)

        # pad input texts by adding zeros at the end. NOTE: does it use zero or other numbers in training code?
        max_text_len = max(len(t) for t in text_tokens)
        text_tokens_padded = torch.zeros((len(texts), max_text_len), dtype=torch.long)
        for i, tokens in enumerate(text_tokens):
            text_tokens_padded[i, :len(tokens)] = tokens
        text_tokens_padded = text_tokens_padded.to(self.device)

        # Add special tokens TODO: check this again
        text_tokens_padded = F.pad(
            text_tokens_padded, (1, 1), value=self.gpt.stop_text_token
        )  # Add stop token at both ends
        text_tokens_padded[:, 0] = self.gpt.start_text_token  # Replace first token

        # Generate GPT codes 
        gpt_codes = self.gpt.generate(
            cond_latents=gpt_cond_latents,
            text_inputs=text_tokens_padded,
            input_tokens=None,
            do_sample=do_sample,
            top_p=top_p,
            top_k=top_k,
        ...
        gpt_code_lengths = (gpt_codes != self.gpt.stop_audio_token).sum(dim=1)

        # Calculate effective lengths
        effective_lengths = [
            (gpt_codes[i] != self.gpt.stop_audio_token).sum() * self.gpt.code_stride_len
            for i in range(len(texts))
        ]

        # Generate latents and audio
        gpt_latents = self.gpt(
                    text_tokens_padded,
                    text_lengths,
                    gpt_codes,
                    torch.tensor(effective_lengths),
                    cond_latents=gpt_cond_latents,
                    return_attentions=False,
                    return_latent=True,
                )
        ...
        wavs = []
        for i in range(len(texts)):
            # Trim latents to effective length
            sample_latents = gpt_latents[i:i+1, :gpt_code_lengths[i], :]
            # if length_scale != 1.0:
                # sample_latents = F.interpolate(
                #     sample_latents.transpose(1, 2),
                #     scale_factor=length_scale,
                #     mode="linear",
                # ).transpose(1, 2)

            # Decode to waveform
            wav = self.hifigan_decoder(sample_latents, g=speaker_embeddings[i:i+1])
            wavs.append(wav.squeeze(0).cpu())
